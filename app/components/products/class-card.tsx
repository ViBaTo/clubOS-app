...
export function ClassCard({ classItem, onView, onReserve }: ClassCardProps) {
  ...
  const isDisabled =
    classItem.status === 'Completa' || classItem.status === 'Cancelada'
  ...
  return (
    <div className='bg-white rounded-xl shadow-sm border-0 p-6 hover:shadow-md transition-shadow duration-200'>
      ...
      <div className='flex items-center justify-between mb-4'>
        ...
        <Badge className={`${getStatusColor(classItem.status)} font-medium`}>
          {classItem.status}
        </Badge>
      </div>
      ...
      <div className='flex items-center gap-3 mb-4'>
        <span className='material-symbols-outlined text-[#1E40AF] text-xl'>
      ...
      {/* Level and court */}
      <div className='flex items-center justify-between mb-4'>
        <Badge className={`${getLevelColor(classItem.level)} font-medium`}>
          {classItem.level}
        </Badge>
