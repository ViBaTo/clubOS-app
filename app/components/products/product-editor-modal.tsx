...
export function ProductEditorModal({
  ...
  const updateCfg = (k: string, v: any) =>
    setForm((p) => ({
      ...p,
      configuration: { ...(p.configuration || {}), [k]: v }
    }))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload: ProductRecord = {
        ...
        classes_included:
          type === 'academy' ? null : form.classes_included ?? null,
        is_active: true,
      ...
      const res = await fetch(
        isCreate ? '/api/products' : `/api/products/${targetId}`,
        {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-2xl'>
        <DialogHeader>
          ...
          {!isCreate && isView && (
            <div className='mt-2'>
              <Button variant='outline' onClick={() => setModeState('edit')}>
                Editar
              </Button>
            </div>
          )}
          ...
          {type === 'academy' ? (
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              <div className='space-y-2'>
                ...
                <Input
                  type='number'
                  value={(form.duration_days ?? '') as any}
                  onChange={(e) =>
                    setForm({
                      ...form,
              ...
              <div className='space-y-2'>
                ...
                <Input
                  value={form.configuration?.instructor ?? ''}
                  onChange={(e) => updateCfg('instructor', e.target.value)}
                  disabled={isView}
                />
              </div>
              <div className='space-y-2'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              ...
              <div className='space-y-2'>
                ...
                <Input
                  type='time'
                  value={form.configuration?.time ?? ''}
                  onChange={(e) => updateCfg('time', e.target.value)}
                  disabled={isView}
                />
              </div>
              <div className='space-y-2'>
                ...
                <Select
                  ...
                  onValueChange={(v) => updateCfg('type', v)}
                  disabled={isView}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value='Clase particular'>
                      Clase particular
                    </SelectItem>
              ...
              <div className='space-y-2'>
                <Label>Pista</Label>
                <Input
                  value={form.configuration?.court ?? ''}
                  onChange={(e) => updateCfg('court', e.target.value)}
                  disabled={isView}
                />
              </div>
              <div className='space-y-2'>
                <Label>Nivel</Label>
                <Select
                  value={form.configuration?.level ?? ''}
                  onValueChange={(v) => updateCfg('level', v)}
                  disabled={isView}
                >
                  <SelectTrigger>
              ...
              <div className='space-y-2'>
                <Label>Estado</Label>
                <Select
                  value={form.configuration?.status ?? ''}
                  onValueChange={(v) => updateCfg('status', v)}
                  disabled={isView}
                >
                  <SelectTrigger>
