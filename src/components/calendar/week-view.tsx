...
export function WeekView({
  ...
  const { dragState, startDrag, endDrag, updateDropZone, executeDrop } =
    useDragDrop({
      events,
      onEventMove
    })
  ...
  const isDropZone = (date: Date, hour: string) => {
    return (
      dragState.dropZone &&
      isSameDay(dragState.dropZone.date, date) &&
      dragState.dropZone.hour === hour
    )
  }
  ...
  return (
    <>
      <div className='flex-1 bg-white overflow-hidden flex flex-col'>
        {/* Header with days */}
        <div
          className='grid border-b border-[#E5E7EB] bg-[#F9FAFB] flex-shrink-0'
          style={{
            gridTemplateColumns: `${timeColumnWidth}px repeat(7, minmax(0, 1fr))`
          }}
        >
          ...
          {weekDays.map((date, index) => {
            ...
            return (
              <div
                key={index}
                className={cn(
                  'p-4 text-center border-r border-[#E5E7EB]',
                  isToday && 'bg-[#1E40AF]/5'
                )}
              >
        ...
        >
          <div
            ...
            style={{
              minHeight: `${hourSlots.length * 60}px`,
              gridTemplateColumns: `${timeColumnWidth}px repeat(7, minmax(0, 1fr))`
            }}
          >
            {/* Time column */}
            <div
              className='border-r border-[#E5E7EB] sticky left-0 bg-white z-20'
            ...
            {weekDays.map((date, dayIndex) => (
              <div
                key={dayIndex}
                className='border-r border-[#E5E7EB] relative'
              >
                {hourSlots.map((hour) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              dragState.draggedEvent?.id === event.id &&
                                'opacity-30'
                            )}
                            style={{
                          ...
                          >
                            ...
                            <div className='text-xs opacity-75 truncate'>
                              {formatTime(new Date(event.fechaInicio))}
                            </div>
                            <div className='text-xs opacity-75 truncate'>
                              {event.pista.nombre}
                            </div>
                      ...
                      <DropZoneIndicator
                        isActive={dragState.isDragging && isDropZoneActive}
                        isValid={isValidDrop}
                      />
                    </div>
                  )
