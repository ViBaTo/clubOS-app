...
export function EnhancedMonthView({
  ...
  const lastTriggeredDirectionRef = useRef<0 | 1 | -1>(0)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      lastTriggeredDirectionRef.current = 0
    }, animationDuration)
  ...
  const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    ...
    const cooldownMs = animationDuration + 150
    ...
    if (
      lastTriggeredDirectionRef.current !== 0 &&
      direction !== lastTriggeredDirectionRef.current &&
      now - lastScrollTimeRef.current < cooldownMs
    ) {
      event.preventDefault()
      return
    }
    ...
    const threshold = horizontalDominant ? 60 : 80

    if (direction === 1 && scrollDeltaRef.current > threshold && now - lastScrollTimeRef.current >= throttleMs) {
      event.preventDefault()
      ...
      scrollDeltaRef.current = 0
      lastTriggeredDirectionRef.current = direction
    } else if (direction === -1 && scrollDeltaRef.current < -threshold && now - lastScrollTimeRef.current >= throttleMs) {
      event.preventDefault()
  ...
  return (
    <>
      <div className="relative flex-1 h-full w-full overflow-hidden bg-white" onWheel={handleWheel}>
        <div
          className={cn(
            "flex h-full w-full flex-col transition-transform duration-[320ms] ease-[cubic-bezier(0.22,1,0.36,1)]",
            baseTransformClass,
          )}
        >
        ...
        {transitionState && overlayTransformClass && (
          <div
            className={cn(
              "pointer-events-none absolute inset-0 flex h-full w-full flex-col transition-transform duration-[320ms] ease-[cubic-bezier(0.22,1,0.36,1)]",
              overlayTransformClass,
            )}
          >
            <div className="pointer-events-auto flex h-full w-full flex-col">
              {renderMonthContent(transitionState.targetDate)}
            </div>
          </div>
        )}
      </div>
